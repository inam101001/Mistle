...
go.Shape.defineFigureGenerator("EndState", (shape, w, h) => {
  const geo = new go.Geometry();

  // Outer ellipse
  geo.add(
    new go.PathFigure(w, h / 2, false).add(
      new go.PathSegment(
        go.SegmentType.Arc,
        0,
        360,
  ...
  geo.add(
    new go.PathFigure(innerW * 1.112, innerH / 2, true).add(
      new go.PathSegment(
        go.SegmentType.Arc,
        0,
        360,
...
go.Shape.defineFigureGenerator("WeakEntity", function (shape, w, h) {
  ...
  // Outer rectangle
  geo.add(
    new go.PathFigure(0, 0, true) // start point
      .add(new go.PathSegment(go.PathSegment.Line, w, 0)) // top edge
      .add(new go.PathSegment(go.PathSegment.Line, w, h)) // right edge
      .add(new go.PathSegment(go.PathSegment.Line, 0, h)) // bottom edge
      .add(new go.PathSegment(go.PathSegment.Line, 0, 0).close()) // left edge
  );

  // Inner rectangle - let's say it's inset by 20 units from the outer rectangle
  var inset = 4;
  geo.add(
    new go.PathFigure(inset, inset, true) // start point
      .add(new go.PathSegment(go.PathSegment.Line, w - inset, inset)) // top edge
      .add(new go.PathSegment(go.PathSegment.Line, w - inset, h - inset)) // right edge
      .add(new go.PathSegment(go.PathSegment.Line, inset, h - inset)) // bottom edge
      .add(new go.PathSegment(go.PathSegment.Line, inset, inset).close()) // left edge
  );

  return geo;
});
go.Shape.defineFigureGenerator("AssociativeEntity", function (shape, w, h) {
  var diamondWidth = w * 0.5;
  var diamondHeight = h * 0.5;

  return (
    new go.Geometry()
      // Rectangle around the diamond
      .add(
        new go.PathFigure(0, 0)
      ...
      // Diamond shape
      .add(
        new go.PathFigure(0.5 * w - diamondWidth, 0.5 * h)
          .add(
            new go.PathSegment(
              go.PathSegment.Line,
              0.5 * w,
...
go.Shape.defineFigureGenerator("WeakRelationship", function (shape, w, h) {
  return new go.Geometry()
    .add(
    ...
    .add(
      new go.PathFigure(0.5 * w, 0.1 * h)
        .add(new go.PathSegment(go.PathSegment.Line, 0.1 * w, 0.5 * h))
        .add(new go.PathSegment(go.PathSegment.Line, 0.5 * w, 0.9 * h))
        .add(new go.PathSegment(go.PathSegment.Line, 0.9 * w, 0.5 * h).close())
    )
    .setSpots(0.25, 0.25, 0.75, 0.75);
});
...
go.Shape.defineFigureGenerator("Multivalued", (shape, w, h) => {
  ...
  // Outer ellipse
  geo.add(
    new go.PathFigure(w, h / 2, true).add(
      new go.PathSegment(
        go.SegmentType.Arc,
        0,
        360,
  ...
  geo.add(
    new go.PathFigure(innerW * 1.053, innerH / 2, true).add(
      new go.PathSegment(
        go.SegmentType.Arc,
        0,
        360,
